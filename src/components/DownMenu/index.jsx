...
const DownMenu = ({ isMenuVisible, toggleMenu }) => {
    ...
    const handleLogout = async () => {
        try {
            const response = await axios.get('http://localhost/Garden_v0.0004/php/logout.php', {
    ...
    const handleDeleteAccount = async () => {
        try {
            const response = await axios.post('http://localhost/Garden_v0.0004/php/deleteUser.php', {}, {
                withCredentials: true
            });
            if (response.data.status === 'success') {
                navigate('/');
